...
export default async function DashboardPage({
  ...
  // Filtered list based on URL param
  const filteredProjects = projects.filter((p) => {
    if (activeFilter === "ACTIVE")  return p.status === "ACTIVE";
    if (activeFilter === "PENDING") return p.status === "PENDING";
    if (activeFilter === "CLOSED")  return p.status === "CLOSED" || p.status === "EXTENDED";
    return true; // ALL
  });

  // Budget / milestone / member stats (always across ALL projects)
    ...
    <div className="min-h-screen bg-[#F5F5F7]">
      ...
      <header className="bg-white border-b border-[#EBEBF0] sticky top-0 z-10">
        <div className="flex items-center gap-4 px-6 h-14">
          ...
          {/* Search */}
          <div className="flex items-center gap-2 bg-[#F5F5F7] border border-[#EBEBF0] rounded-lg px-3 py-1.5">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9999AA" strokeWidth="2">
              <circle cx="11" cy="11" r="8" />
              <line x1="21" y1="21" x2="16.65" y2="16.65" />
            </svg>
            <input
              className="bg-transparent text-[12px] text-[#555570] outline-none w-44 placeholder:text-[#9999AA]"
      ...
      <main className="px-6 py-6 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-7">
          {[
            ...
            <div
              key={s.label}
              className="bg-white border border-[#EBEBF0] rounded-xl p-4 relative overflow-hidden"
            >
              <div
              ...
              <div className="text-[22px] font-semibold text-[#1A1A2E] tracking-tight leading-none">
                {s.value}
              </div>
        ...
        {/* ── Filter tabs (functional via URL params) ── */}
            ...
            <Link
              ...
              className={`text-[12px] font-medium px-3 py-1.5 rounded-full border transition-colors ${
                activeFilter === tab.key
                  ? "bg-[#EEF0FF] border-[#C5C0F5] text-[#5B4FE9]"
                  : "bg-white border-[#EBEBF0] text-[#555570] hover:bg-[#F5F5F7]"
              }`}
            >
              {tab.label}
            </Link>
        ...
        {filteredProjects.length === 0 ? (
          <div className="bg-white border border-[#EBEBF0] rounded-2xl p-16 text-center">
            ...
            <p className="text-[13px] text-[#9999AA]">
              {activeFilter === "ALL" && currentUser?.role === "PI"
                ? "Create your first ANRF project to get started."
                : activeFilter === "ALL"
                ? "You have not been added to any projects yet."
                : `You have no projects with ${activeFilter.toLowerCase()} status.`}
            </p>
          </div>
        ) : (
