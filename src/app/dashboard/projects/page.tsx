...
export default async function ProjectsPage() {
  ...
  const projects = rawProjects.map((p) => ({
    ...
    // true only if the logged-in user is the PI of this specific project
    isPI: p.pi?.id === currentUser?.id,
  }));
      ...
      {projects.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {projects.map((project) => (
            <ProjectCard key={project.id} project={project} isPI={project.isPI} />
          ))}
        </div>
      )}
    </div>
